...
_RUNTIME_VERSION = "3b7d592b62a8a8f6"
_RUNTIME_SOURCE = r'''
from __future__ import annotations
...
def code_of(value):
    ...
    if not re.fullmatch(r"\d{6}", code):
        raise ValueError("6자리 국내주식 종목코드 불일치")
    return code
...
class Tick:
    ...
    def parse(cls, message, received):
        ...
        if header.get("tr_cd") != "oc":
            return None
        code = code_of(body["code"])
        if header.get("tr_key") and header["tr_key"] != code:
...
class NhBroker:
    ...
    def balance(self):
        ...
        holdings = []
        for page in pages:
            for row in rows(page.get("Output_1")):
                qty = integer(row["itg_bnc_qty"])
                if qty <= 0:
                    continue
                holdings.append(dict(code=code_of(row["iem_cd"]), name=str(row.get("iem_nm", "")),
                                     qty=qty, avg=number(row["phs_pr"]), price=number(row["now_pr"])))
    ...
    def load_universe(self):
        ...
        for row in records:
            ...
            if not re.fullmatch(r"\d{6}", code) or str(row["sMarket"]).strip() not in {"1", "4"}:
                continue
...
class Engine:
    ...
    def connect(self, account):
        with self.actions:
            ...
            self.broker.history(self.clock().date().isoformat())
    ...
    def set_mode(self, mode):
        ...
        with self.actions:
            if mode != "STOPPED" and not self.account:
    ...
    def scan_loop(self):
        ...
        while not self.stop_event.is_set():
            try:
                if self.mode != "RUNNING" or not market_time(self.clock(), True) or not self.account:
...
st.title("나무 자동매매")
st.caption("계좌·작동 현황 강화본 · 2026.09.09 · 실전 계좌")
...
def dashboard():
    ...
    if data["account_holdings"]:
        for h in data["account_holdings"]:
            ...
            st.markdown(f'<div class="nh-holding"><div class="nh-holding-head"><strong>{title}</strong>'
                        ...
                        f'<span>보유 {h["qty"]:,}주</span><span>매입가 {won(h["avg"])}</span>'
    ...
    with tabs[2]:
        ...
        import json
        report = {k:data[k] for k in ("mode","status","error","balance_error","balance_at","cycle_at","scan_at","ws_state","ws_error","last_tick","tick_count","invalid_ticks","scan_source","scan_done","scan_total","last_scan_error")}
