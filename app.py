...
api_total_asset = to_int(first_of(summary, [
    "tot_aset_amt", "tot_evlu_amt", "tot_evlt_amt", "tot_asset_amt"
], 0))
api_eval_amt = to_int(first_of(summary, [
    "evlu_amt", "evlt_amt", "tot_evlu_stk_amt", "stock_evlu_amt"
], 0))
api_buy_amt = to_int(first_of(summary, [
    "pchs_amt", "tot_pchs_amt", "buy_amt"
], 0))
api_pnl = to_int(first_of(summary, [
    "evlu_pfls_amt", "evlt_pls_amt", "eal_pls_amt", "pnl_amt"
], 0))
...
display_return = (display_pnl / display_buy * 100) if display_buy > 0 else 0.0
...
if account_value == 0 and isinstance(bal, dict) and bal:
    st.warning("NH 잔고 응답은 왔지만 금액 필드가 현재 화면과 맞지 않습니다. 아래 진단값으로 정확히 맞출 수 있습니다.")
    with st.expander("🔎 잔고 응답 진단 (계좌번호·인증정보 제외)", expanded=True):
        st.json(safe_balance_debug(bal))
