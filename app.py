...
_RUNTIME_VERSION = "9c035fdf3eba36a9"
_RUNTIME_SOURCE = r'''
from __future__ import annotations
...
class Engine:
    ...
    def desired_codes(self):
        with self.lock:
            if self.mode == "STOPPED" or not market_time(self.clock()):
                return []
    ...
    def snapshot(self):
        ...
        with self.lock:
            ...
            data = dict(mode=self.mode, status=self.status, error=self.error, ws_state=self.ws_state,
                        ...
                        last_signal=self.last_signal, scan_done=self.scan_done, scan_total=self.scan_total,
...
st.title("나무 자동매매")
st.caption("5분봉 관측 · 거래량 100만 주 이상 · 실전 계좌 · 2파일 통합본")
