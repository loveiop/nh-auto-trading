    ...
    while True:
        try:
            ...
            for c in candidates:
                ...
                scalp_score, scalp_reason, scalp_detail = score_scalp(code)
                ...
                with runtime_lock:
                    runtime["scalp_status"][code] = {
                        ...
                        "score": scalp_score,
                        "reason": scalp_reason,
                        "rise5": float(scalp_detail.get("rise5", 0) or 0),
                        "momentum": float(scalp_detail.get("mom_short", 0) or 0),
                        "vol_delta": int(scalp_detail.get("vol_delta", 0) or 0),
                        "updated": now_kst().strftime("%H:%M:%S"),
                    }

                # 현재 5분봉 + 순간 가격/거래량 가속 점수로 즉시 판정
                if scalp_score < int(settings["buy_score"]):
                    continue
                ...
                reason = (
                    f"스캘핑 {scalp_score}점 · {scalp_reason} · "
                    f"일봉후보 {c['score']}점 · 거래대금 {c['turnover']:,}원"
                )
                set_pending(code)
                try:
                    send_market_order(code, qty, "BUY", reason, scalp_score, price)
                    with runtime_lock:
