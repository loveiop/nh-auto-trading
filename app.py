        ...
        try:
            ...
            with runtime_lock:
                cand = sorted(
                    ...
                    key=lambda x: (x.get("turnover",0), x.get("volume",0), x.get("score",0)),
                    reverse=True
                )
        ...
        try:
            ...
            for code in batch:
                try:
                    ...
                    with runtime_lock:
                        runtime["scan_checked"] += 1
                        if volume >= 1_000_000:
                            runtime["scan_volume_1m"] += 1
                        if volume >= 1_000_000 and turnover >= int(settings["min_turnover"]):
                            runtime["scan_liquid"] += 1
            ...
            if cursor >= len(codes):
                ...
                cycle += 1
                with runtime_lock:
                    runtime["scan_checked"] = 0
                    runtime["scan_volume_1m"] = 0
                    runtime["scan_liquid"] = 0
                    runtime["scan_last_cycle"] = now_kst().strftime("%H:%M:%S")
...
st.title("⚡ NH Auto Trader · LIVE SCALPING v16 SIMPLE")
...
with runtime_lock:
    scan_checked = int(runtime.get("scan_checked",0))
    scan_volume_1m = int(runtime.get("scan_volume_1m",0))
    scan_liquid = int(runtime.get("scan_liquid",0))
    scan_last_cycle = runtime.get("scan_last_cycle","")
...
st.markdown(
    f"**현재 순환 조회 {scan_checked:,}종목 → 거래량 100만주 이상 {scan_volume_1m:,}종목 "
    f"→ 유동성 통과 {scan_liquid:,}종목 → 실시간 감시 {len(ws_codes)}종목**"
)
if scan_last_cycle:
    st.caption(f"최근 전체시장 순환 완료: {scan_last_cycle}")
...
# Settings — 사용자 조절값은 핵심 5개만 노출
with st.expander("⚙️ 간단 설정", expanded=True):
    st.caption("나머지 안전값은 스캘핑 권장값으로 자동 적용됩니다.")
    capital = st.number_input("투자금", 100_000, 100_000_000, int(settings["capital"]), 100_000)
    per_stock = st.number_input("종목당 매수금액", 10_000, 50_000_000, int(settings["per_stock"]), 10_000)
    stop_loss = st.slider("손절 %", 0.3, 3.0, float(settings["stop_loss"]), 0.1)
    take_profit = st.slider("익절 %", 0.5, 5.0, float(settings["take_profit"]), 0.1)
    buy_score = st.slider("매수 최소점수", 50, 100, int(settings["buy_score"]), 5)

    st.caption("자동 적용: 최대 3종목 · 트레일링 0.6% · 재매수 10분 대기 · 거래량 100만주 이상 · 실시간 감시 10종목 · 화면 30초 갱신")
    ...
    if st.button("💾 설정 저장", use_container_width=True):
        ...
        set_settings(n)
        log("SETTINGS", "간단 설정 저장 · 숨김 안전값 자동 적용")
        st.success("저장했습니다.")
        st.rerun()
