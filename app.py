...
account_value = int(cash) + int(total_eval_value)

x1,x2,x3,x4 = st.columns(4)
x1.metric("계좌 금액", f"{account_value:,}원")
x2.metric("주문가능금액", f"{cash:,}원")
x3.metric("주식 평가금액", f"{total_eval_value:,}원")
x4.metric("평가손익", f"{total_eval_pnl:+,}원")

y1,y2,y3 = st.columns(3)
y1.metric("보유종목", f"{len(holds)}개")
buy_amt,trades,_ = today_stats()
y2.metric("오늘 자동매수 요청액", f"{buy_amt:,}원")
y3.metric("오늘 자동주문 요청", f"{trades}건")
