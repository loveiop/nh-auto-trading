...
def score_5m(code):
    """완성된 5분봉 기준 100점 단타 진입 점수.
    MA3>MA6 30점 / 직전봉 대비 상승 25점 /
    거래량 1.5배 25점 / 직전 3봉 고가 돌파 20점
    """
    now = now_kst()
    current_key = now.replace(minute=(now.minute//5)*5, second=0, microsecond=0).strftime("%Y-%m-%d %H:%M")
    with runtime_lock:
        raw = list(runtime["minute5"].get(code, {}).get("bars", []))

    # 현재 진행 중인 봉은 신호 계산에서 제외하여 봉 완성 전 흔들림을 줄임
    bars = [b.copy() for b in raw if b.get("bucket") != current_key]
    if len(bars) < 6:
        return 0, f"5분봉 준비중 {len(bars)}/6", len(bars)
    ...
    prev3 = df.iloc[-4:-1]
    prev_vol3 = vols.iloc[-4:-1].mean()

    score = 0
    reasons = []
    if ma3 > ma6:
        score += 30; reasons.append("5분 MA3>MA6 +30")
    if closes.iloc[-1] > closes.iloc[-2]:
        score += 25; reasons.append("5분 상승모멘텀 +25")
    if prev_vol3 > 0 and float(z["volume"]) > prev_vol3 * 1.5:
        score += 25; reasons.append("5분 거래량급증 +25")
    if len(prev3) == 3 and float(z["close"]) >= float(prev3["high"].max()):
        score += 20; reasons.append("5분 3봉고점돌파 +20")

    return score, " / ".join(reasons) if reasons else "5분 조건 미충족", len(bars)
    ...
    while True:
        try:
            ...
            for c in candidates:
                ...
                # 일봉 점수는 '후보 선별'에만 사용하고 실제 진입은 5분봉으로 판단
                pr = current_price(code)
                o = pr.get("Output_0") if isinstance(pr.get("Output_0"),dict) else {}
                price = to_int(first_of(o, ["stck_prpr","now_pr","prpr"], c["close"]))
                cum_volume = to_int(first_of(o, ["acml_vol","volume"], 0))
                ...
                m5_score, m5_reason, m5_count = score_5m(code)

                # 6개 완성봉(약 30분) 확보 후부터 실전 진입 판정
                if m5_count < 6:
                    continue
                if m5_score < int(settings["buy_score"]):
                    continue
                ...
                reason = (
                    f"5분봉 {m5_score}점 · {m5_reason} · "
                    f"일봉후보 {c['score']}점 · 거래대금 {c['turnover']:,}원"
                )
                set_pending(code)
                try:
                    send_market_order(code, qty, "BUY", reason, m5_score, price)
                    with runtime_lock:
...
with st.expander("⚙️ 금액 · 전략 · 안전설정", expanded=False):
    ...
    with r:
        buy_score = st.slider("5분봉 매수 최소점수",50,100,int(settings["buy_score"]),5)
