...
_RUNTIME_VERSION = "093a58ff972ea547"
_RUNTIME_SOURCE = r'''
from __future__ import annotations
...
STRATEGY_VERSION = "2026.09.09-r3"
...
class Tape:
    ...
    def signal(self, settings, current):
        ...
        if settings.net_reward_risk() < settings.min_net_reward_risk:
            return dict(score=0, reason="예상 비용을 반영하면 목표수익 대비 손절 위험이 큼", eligible=False)
        ...
        with self.lock:
            for p in positions:
                ...
                if tape.latest.bid <= 0:
                    return realized, False
                unrealized += p["qty"] * (tape.latest.bid-p["avg"]-p["avg"]*self.settings.cost_allowance_pct/100)
    ...
    def submit(self, code, side, reason):
        with self.actions:
            ...
            if side == "BUY":
                ...
                committed_risk = sum(p["qty"]*p["avg"]*risk_rate for p in owned)
            ...
            try:
                def permit():
                    with self.lock:
                        ...
                        if side == "BUY" and not tape.signal(self.settings, stamp)["eligible"]:
                            raise OrderStopped("통신 대기 중 진입 조건이 바뀌어 주문을 전송하지 않았습니다.")
    ...
    def step(self):
        ...
        for position in positions:
            ...
            if self.store.get("loss_stop_"+self.account) == current.date().isoformat():
                ...
                reason = f"익절 {gain:.2f}%"
            elif peak >= avg*(1+(self.settings.trailing+self.settings.cost_allowance_pct)/100) and (tick.bid/peak-1)*100 <= -self.settings.trailing:
...
st.title("나무 자동매매")
st.caption("연결·정지·전략 복기 개선본 R3 · 2026.09.09 · 실전 계좌")
    ...
    with tabs[2]:
        ...
        if review and review["count"]:
            ...
            st.dataframe([dict(진입=t["opened"][:19], 청산=t["closed"][:19], 코드=t["code"],
                               ...
                               진입점수=t["signal"].get("score"), 손절=t["settings"].get("stop_loss"),
        ...
        with st.expander("이번에 반영한 매매·검증 원칙"):
            ...
            st.write("3. 매도 가능한 현재 호가를 기준으로 손절·익절을 판단하며, 비용을 감안해 이익 보호 조건을 적용합니다.")
            st.write(f"4. 당일 청산 거래가 {settings.consecutive_loss_limit}회 연속 손실이면 신규매수를 정지합니다.")
            st.write("5. 진입 당시의 점수·설정·호가와 실제 체결 결과를 함께 기록합니다. 조건을 바꿀 때는 이전 기록과 이후 결과를 분리해 비교해야 합니다.")
            st.caption("이 수치는 운영을 위한 초기 기준이며, 과거 수익으로 최적화한 값이 아닙니다. 급변·거래정지·통신 중단 시 실제 손실은 예상 손실예산을 초과할 수 있습니다.")
            st.markdown("연구 근거: [거래비용과 체결 위험 · FINRA](https://www.finra.org/rules-guidance/rulebooks/finra-rules/2270), [반복 최적화의 과적합 · Bailey 외](https://www.davidhbailey.com/dhbpapers/backtest-prob.pdf). 해당 자료가 이 프로그램의 수익성을 입증하는 것은 아닙니다.")
        if data["connected"]:
